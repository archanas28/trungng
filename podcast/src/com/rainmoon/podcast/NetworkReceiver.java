...
public class NetworkReceiver extends BroadcastReceiver {
  ...
  public void onReceive(Context context, Intent intent) {
    ...
    // If the userpref is Wi-Fi only, checks to see if the device has a Wi-Fi
    // connection.
    if (NetworkActivity.WIFI.equals(NetworkActivity.sPref) && networkInfo != null
      ...
      // Otherwise, the app can't download content--either because there is no
      // network
      // connection (mobile or Wi-Fi), or because the pref setting is WIFI,
      // and there
      // is no Wi-Fi connection.
      // Sets refreshDisplay to false.
    } else {
      NetworkActivity.refreshDisplay = false;
      Toast.makeText(context, R.string.lost_connection, Toast.LENGTH_SHORT)
          .show();
    }
  }
}