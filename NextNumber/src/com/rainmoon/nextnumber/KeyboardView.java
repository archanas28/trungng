...
import com.rainmoon.nextnumber.R;
...
public class KeyboardView extends View {
  ...
  private MainActivity calculator;
  ...
  static final char[][] DIGITS = { { '7', '8', '9', '+', MINUS, SQRT },
      { '4', '5', '6', TIMES, DIV, '^' }, { '1', '2', '3', '0', '!', 'C' }, };
  ...
  protected void onSizeChanged(int w, int h, int ow, int oh) {
    ...
    for (int line = 0; line < nLine; ++line) {
      ...
      for (int col = 0; col < nCol; ++col) {
        ...
        if ((col > 0 && c == lineKeys[col - 1])
            || (line > 0 && c == keys[line - 1][col])) {
          continue;
        }
        // span columns
        float cw = cellw;
        if (col < nCol - 1 && c == lineKeys[col + 1]) {
          int cnt = 2;
          for (int i = col + 2; i < nCol; i++) {
            if (c == lineKeys[i]) {
              cnt++;
            } else
              break;
        ...
        float ch = cellh;
        if (line < nLine - 1 && c == keys[line + 1][col]) {
          int cnt = 2;
          for (int i = line + 2; i < nLine; i++) {
            if (c == keys[i][col]) {
              cnt++;
            } else {
              break;
        ...
        default:
          // textPaint.setColor(('0' <= c && c <= '9') ? 0xffffff00 :
          // 0xffffffff);
    ...
    /*
     * linePaint.setStrokeWidth(0); linePaint.setColor(0xff000000); for (int
     * line = 0; line <= nLine; ++line) { final float y = getY(line);
     * canvas.drawLine(0, y, width, y, linePaint); } for (int col = 0; col <=
     * nCol; ++col) { final float x = getX(col); canvas.drawLine(x, 0, x,
     * height, linePaint); }
     */
  }

  private void drawDrawable(Canvas canvas, int id, float x, float y, float cw,
      float ch) {
  ...
  private void invalidateCell(int line, int col) {
    ...
    // log("invalidate " + x + ' ' + y + ' ' + ((int)x1) + ' ' + ((int)y1) + ' '
    // + x2 + ' ' + y2);
  }
}
