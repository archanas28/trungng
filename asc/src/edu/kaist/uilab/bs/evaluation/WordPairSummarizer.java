...
/**
 * Summarize summaries using adject-noun pairs.
 * <p>
 ...
 * @author trung
 */
public class WordPairSummarizer {
  private static final MaxentTagger tagger = MaxentTaggerSingleton
      .getInstance();
   ...
   * @return
   */
  private ObjectToCounterMap<WordPair> getTermFrequency(String content,
   ...
   * @return
   */
  public ObjectToCounterMap<WordPair> getWordPairsWithTermFrequency(
      String content) {
    return getTermFrequency(content, null);
  }

  /**
   * Returns the list of top word pairs ranked by term frequency for the given
   * content.
   * 
   * @param content
   *          reviews of an entity
   * @param num
   *          how many word pairs to return
   * @return
   */
  public List<WordPair> getTopWordPairs(String content, int num) {
    ObjectToCounterMap<WordPair> counter = getWordPairsWithTermFrequency(content);
    int size = num > counter.size() ? counter.size() : num;
    return counter.keysOrderedByCountList().subList(0, size);
  }
   ...
   * @return
   */
  private ArrayList<WordPair> getWordPairs(ArrayList<TaggedWord> tWords) {
  ...
  public void printTopWordPairs(PrintWriter out,
    ...
    for (Entry<String, ArrayList<Review>> entry : map.entrySet()) {
      ...
      if (numReviews < maxReviews && numReviews >= minReviews) {
        ...
        ObjectToCounterMap<WordPair> tf = getWordPairsWithTermFrequency(allReviews);
  ...
  /**
   * Prints the top aspects of an entity given its word pair
   * <code>counter</code>.
   * 
   * @param out
   * @param counter
   ...
   * @return
   */
  String aggregateReviews(ArrayList<Review> reviews) {
  ...
  private void summarizeUsingTermFrequency(WordPairSummarizer wp,
  ...
  public static void main(String args[]) throws Exception {
    ...
    WordPairSummarizer wp = new WordPairSummarizer();
    wp.summarizeUsingTermFrequency(wp, map, dir);
  }
}
