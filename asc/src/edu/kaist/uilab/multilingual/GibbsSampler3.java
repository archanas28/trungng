 ...
 */
public class GibbsSampler3 implements ObjectiveFunction {
//  static final String workingDir = "C:/datasets/asc/ldatest/ElectronicsReviews1";
//  static final String workingDir = "C:/datasets/asc/ldatest/obama";
...
//  static final String workingDir = "C:/datasets/asc/ldatest/europarl";
  ...
  public static void main(String args[]) throws IOException {
    ...
    String outputDir = String.format("%s/(v3)T%d-I%d-A%.2f-L%.1f-Phi%.1f", workingDir,
        numTopics, numIters, alpha, mLamdaWord, edgeWeight);
    ...
    GibbsSampler3 sampler = new GibbsSampler3(numTopics, symbol, docs,
   ...
   * @param outputDir
   */
  public GibbsSampler3(int numTopics, ArrayList<String> symbol,
  ...
  public void doGibbsSampling(int numIters, int burnIn, int optimizationInterval) {
    ...
    for (int iter = 0; iter < numIters; iter++) {
      ...
      // allows French documents to be sampled (100 iters) before optimization
      // no, actually it optimizes beta at half iterations without the graph
      if (!graphInit && realIter * 2 > numIters) {
  ...
  public double computeFunction(double[] x) throws InvalidArgumentException {
    ...
    kvTerm *= edgeWeight / (2 * mLamdaTopic * mLamdaTopic);
    double vTerm = 0;
    for (int i = 0; i < mEffectiveVocabSize; i++) {
  ...
  public double[] computeGradient(double[] x) throws InvalidArgumentException {
    ...
    for (int k = 0; k < mNumTopics; k++) {
      for (int i = 0; i < mEffectiveVocabSize; i++) {
        ...
        grads[idx] *= edgeWeight / (mLamdaTopic * mLamdaTopic);
