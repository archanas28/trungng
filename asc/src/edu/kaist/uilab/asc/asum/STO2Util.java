...
public class STO2Util {
  ...
  public static DoubleMatrix[] calculatePhi(IntegerMatrix[] matrixSWT,
      int[][] sumSTW, double[] betas, double[] sumBeta,
      List<TreeSet<Integer>> sentiWordsList) { // Beta
    ...
    for (int w = 0; w < numWords; w++) {
      ...
      for (int s = 0; s < numSenti; s++) {
        if (sentiWordsList.get(s).contains(w))
          wordLexicons[w] = s;
      }
    }
    for (int s = 0; s < numSenti; s++) {
      ...
      for (int w = 0; w < numWords; w++) {
        for (int t = 0; t < numTopics; t++) {
          double beta;
          if (wordLexicons[w] == null)
            beta = betas[0];
          else if (wordLexicons[w] == s)
            beta = betas[1];
          else
            beta = betas[2];

          double value = (matrixSWT[s].getValue(w, t) + beta)
              / (sumSTW[s][t] + sumBeta[s]);
  ...
  public static DoubleMatrix[] calculateTheta(IntegerMatrix[] matrixSDT,
    ...
    for (int s = 0; s < numSenti; s++) {
      ...
      for (int d = 0; d < numDocs; d++) {
        for (int t = 0; t < numTopics; t++) {
          double value = (matrixSDT[s].getValue(d, t) + alpha)
              / (sumDST[d][s] + sumAlpha);
